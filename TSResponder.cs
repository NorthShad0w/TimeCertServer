...
namespace LibTimeStamp
{
    ...
    {
        readonly X509Certificate  x509Cert;
        readonly AsymmetricKeyParameter priKey;
        readonly IStore<X509Certificate> certs;
        readonly string hashAlg;
        public TSResponder(byte[] x509Cert, byte[] priKey, string hashAlg)
        {
            ...
            TextReader privakeyReader = new StreamReader(new MemoryStream(priKey));
            PemReader pemReader = new PemReader(privakeyReader);
            ...
            this.certs =  CollectionUtilities.CreateStore(certList);
        ...
        private byte[] RFC3161(byte[] bRequest,DateTime signTime,BigInteger biSerial)
        {
            ...
            signedAttributes.Add(new Attribute(CmsAttributes.SigningTime, new DerSet(new DerUtcTime(signTime,2049))));
            ...
            timeStampTokenGenerator.SetCertificates(certs);
